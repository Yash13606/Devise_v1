...
npx concurrently -k -p "[{name}]" -n "FRONTEND,MCP-GATEWAY,API,AGENT" -c "cyan,magenta,green,yellow" "cd frontend && npm run dev" "cd mcp-gateway && npx ts-node server.ts" "python -m uvicorn api.index:app --reload --port 8000" "cd devise-agent/devise-eye && python main.py"
